...
let
  ...
  lock-transparent =
    launchPrefix + "${lib.getExe (pkgs.callPackage ./swaylock/lock-transparent.nix args)}";
  ...
  term_tmux_scratchpad =
    launchPrefix
    + "${xdg-terminal-exec} -- ${lib.getExe config.programs.sesh.package} connect \"scratchpad 󱞂 \"";
